...
## 2024-06-15 - Prevent Path Resolution Inconsistency in External Execution
**Vulnerability:** Found `MainWindow.OpenWithDefaultProgram` was vulnerable to path resolution inconsistency because it validated a file path using `File.Exists(path)` (which resolves relative paths against the application's current working directory) but subsequently executed it via `Process.Start` configured with `WorkingDirectory = Environment.SpecialFolder.System` (which would execute relative paths against `C:\Windows\System32`).
**Learning:** If a relative file path (like `malicious.exe`) is provided, the application might check for safety against `[AppDir]\malicious.exe` but unknowingly execute `C:\Windows\System32\malicious.exe`, potentially bypassing intended file-based security checks.
**Prevention:** To prevent path resolution inconsistencies (TOCTOU-like vulnerabilities) when verifying a file's existence with `File.Exists(path)` prior to launching it via `Process.Start` with a custom `WorkingDirectory`, always resolve the user-provided path to its absolute path using `Path.GetFullPath(path)` before performing any security checks and execution.
## 2024-06-15 - Process Injection Fix via Allowlist
...
## 2024-08-10 - [Local Exception Handling for Security Executions]
**Vulnerability:** Execution paths intended to enforce security (like `OpenWithDefaultProgram`) throw specific exceptions to block execution, but if unhandled by the UI caller, they bubble up to the global Windows Forms handler, crashing the UI sequence or violating CWE-209 by exposing full technical details.
**Learning:** Security validations that result in thrown exceptions must be explicitly caught by the immediate caller (especially UI event handlers) to ensure the application fails securely and gracefully provides contextual feedback to the user without abruptly terminating or relying on a generic global catch.
**Prevention:** When implementing or calling security enforcement functions that rely on throwing exceptions to block dangerous actions, always wrap the calls in strict local `try-catch` blocks that handle expected specific security exceptions (like `InvalidOperationException` or `UnauthorizedAccessException`) safely.
## 2026-08-18 - [Local Exception Handling for Link Labels]
**Vulnerability:** Unhandled exceptions in UI link click handlers (like `labelGPL_LinkClicked`) opening URLs via `Process.Start`.
**Learning:** When using methods like `OpenUrlSafe` that throw specific validation exceptions (like `InvalidOperationException` or `Win32Exception`), failing to catch them in the event handler allows the error to bubble up to the global Windows Forms handler, leading to a crash and exposing technical details via a stack trace (CWE-209).
**Prevention:** Always wrap external process invocations originating from UI events in a local `try/catch` block that gracefully displays a user-friendly `MessageBox` for expected exceptions (and logs generic ones securely), preventing information disclosure.
## 2026-08-20 - [Local Exception Handling for Win32Exception]
...
**Vulnerability:** Maliciously crafted Word or Excel packages can expand structural or core-property parts far beyond their compressed size, and oversized package properties can consume excessive memory.
**Learning:** `OpenSettings.MaxCharactersInPart` does not protect `OpenXmlPackage.PackageProperties`. Title, Creator, and Created are read from the core-properties part directly, so the document is never opened.
**Prevention:** Bound the physical package size, entry count, the decompressed root relationships part, the decompressed core-properties part, and each metadata value. Resolve the core-properties target inside the package and reject external or parent-segment targets before inflating them. Prohibit DTD processing, disable external XML resolution, and report malformed XML as `InvalidDataException`.
