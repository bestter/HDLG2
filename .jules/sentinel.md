...
## 2024-08-17 - Catch InvalidOperationException in UI Handlers Calling Security Functions
**Vulnerability:** The application was vulnerable to CWE-209 (Information Disclosure) and silent failures because the `RunExportAsync` method in `MainWindow.cs` called `OpenWithDefaultProgram(savePath)`, a security enforcement function that throws `InvalidOperationException` for blocked extensions, without explicitly catching it.
**Learning:** Security validations that result in thrown exceptions (e.g., to block dangerous file extensions) must be explicitly caught by the immediate caller (especially UI event handlers). If unhandled, they fall back to a generic `catch (Exception ex)` block, which logs the error but fails to provide the required secure UI feedback, violating the Fail Securely principle.
**Prevention:** When calling functions that enforce security policies by throwing specific exceptions (like `InvalidOperationException` for blocked file extensions), explicitly wrap the call in a local `catch` block that handles the expected specific security exception safely and provides localized, secure feedback to the user via UI components like `MessageBox`.
