...
## 2024-06-25 - [Prevent Process Injection via UseShellExecute]
**Vulnerability:** Execution of URLs or files via `Process.Start` with `UseShellExecute = true` can be susceptible to command injection if the URL or file path string contains special characters like `&` or `"`, because Windows Shell Execute may interpret the string incorrectly and execute unintended commands.
**Learning:** Even hardcoded URLs or file paths passed to `Process.Start` with `UseShellExecute = true` should be handled carefully because `ShellExecute` behavior is complex and can be bypassed.
**Prevention:** Always use `UseShellExecute = false` and launch files or URLs via a designated explicit executable like `explorer.exe` (on Windows) or `xdg-open` (on Linux), passing the URL or file path as a double-quoted argument. This guarantees the payload is treated as a literal argument to the launcher rather than a command to be parsed by the shell.
