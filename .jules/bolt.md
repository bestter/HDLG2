...
## 2026-08-20 - Avoid DateTime.ToString allocations in Utf8JsonWriter
**Learning:** Calling `.ToString("O", CultureInfo.InvariantCulture)` on a `DateTime` before passing it to `Utf8JsonWriter.WriteString` or `WriteStringValue` allocates an unnecessary string on the heap for every single date serialized. `Utf8JsonWriter` natively supports `DateTime` and writes the ISO-8601 formatted string directly to the UTF-8 buffer without heap allocations.
**Action:** When serializing JSON using `System.Text.Json.Utf8JsonWriter`, always pass `DateTime` instances directly to the write methods instead of calling `.ToString()` to eliminate string allocations in hot paths like directory tree serialization.
