...
## 2026-07-11 - Avoid list capacity bloat when separating FileSystemInfos
**Learning:** When trying to eliminate array resizing allocations by pre-sizing `List<DirectoryInfo>` and `List<FileInfo>` collections, passing `GetFileSystemInfos().Length` to *both* `.Capacity` properties causes severe memory bloat. A directory with 10,000 files and 2 subdirectories will incorrectly pre-allocate a 10,002-item backing array for the directories list, wasting memory. Furthermore, replacing lazy `EnumerateFileSystemInfos` with eager `GetFileSystemInfos` creates a massive array that completely negates any intended optimization.
**Action:** When optimizing file system enumeration in C# to reduce `List<T>` resizing allocations, do not use `DirectoryInfo.GetFileSystemInfos().Length` to pre-size separate, strongly-typed collections (like `List<DirectoryInfo>` and `List<FileInfo>`). This causes massive memory bloat and capacity over-allocation, as the list capacity is sized to the combined total of files and directories. Instead, use `EnumerateDirectories()` and `EnumerateFiles()` for separated iteration without upfront combined array allocation, or, when `GetFileSystemInfos()` is required for atomic retrieval, count the separate items via an initial pass before instantiating exact-sized arrays instead of dynamic lists.

## 2026-07-14 - Preserve XML schema when optimizing early returns
**Learning:** When refactoring C# XML serialization logic (e.g., extracting early returns to skip processing empty collections), it is critical to distinguish between a collection that is `null` versus one that is empty (`.Count == 0`). If the original code emits an empty XML tag (like `<ExtentedProperties></ExtentedProperties>`) for empty collections, adding an early return like `if (collection == null || collection.Count == 0)` will omit the tag entirely, breaking the XML schema for downstream parsers.
**Action:** When extracting early returns around XML serialization blocks, carefully check the original null/empty guarding logic. If the original block only guarded against `null` (e.g., `if (file.Properties != null)`), the early return must strictly be `if (file.Properties == null) return;` to preserve the generation of empty tags.
...
## 2026-08-20 - Eliminate heap allocations in Utf8JsonWriter DateTime serialization
**Learning:** Calling `.ToString("O", CultureInfo.InvariantCulture)` on a `DateTime` before passing it to `Utf8JsonWriter.WriteString` or `WriteStringValue` allocates a new string on the heap. When generating a JSON export for thousands of files and directories, this creates massive amounts of unnecessary garbage collection overhead.
**Action:** To eliminate heap allocations when writing `DateTime` values to JSON with `Utf8JsonWriter` in C#, avoid calling `.ToString("O", CultureInfo.InvariantCulture)`. Instead, pass the `DateTime` object directly to `WriteString` or `WriteStringValue`, which natively formats it to the ISO-8601 extended format directly into the underlying UTF-8 buffer.
