...
## 2026-10-25 - Avoid O(n) array redundant traversal logic in UI
**Learning:** `fsInfos.Length` pre-sizing of `TreeNode` arrays leads to O(n) calculation iterations traversing the same sequence, only to allocate length variables and reiterate immediately after. By combining enumeration into single accumulation loop logic initialized via heuristics, unnecessary processing loops are avoided.
**Action:** Consolidate redundant enumeration loops calculating sub-elements by leveraging statically pre-sized structures or collections dynamically reallocating using safe heuristics like `fsInfos.Length / 2` and executing single path loop traversal.
