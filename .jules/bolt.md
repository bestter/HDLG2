...
## 2024-11-20 - N+1 Sequential I/O Pattern in File Enumeration
**Learning:** Sequential asynchronous operations (`await Task` inside a `foreach` loop) over a large dataset (like directory files) creates a severe N+1 latency bottleneck where overall time is the sum of all individual operations.
**Action:** When iterating over collections containing I/O bound tasks, transition from sequential `await` to throttled concurrency using `System.Threading.SemaphoreSlim` and `Task.WhenAll()`. Crucially, when processing results into non-thread-safe collections (like `List<T>`), explicitly protect the write operation with a `lock` block.
